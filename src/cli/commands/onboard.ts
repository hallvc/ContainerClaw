...
export function buildDotenvContent(apiKey: string): string {
  const lines = [
    "# ContainerClaw configuration",
    "# Generated by: containerclaw onboard",
...
export async function writeOnboardFiles(
  ...
  // Write .env with secrets (don't overwrite existing)
  if (apiKey) {
    const dotenvPath = join(dataDir, ".env");
    const wroteDotenv = await writeFileIfNotExists(
      dotenvPath,
      buildDotenvContent(apiKey),
    );
    if (wroteDotenv) {
      console.log(`  Created ${dotenvPath}`);
    } else {
      console.log(`  Skipped ${dotenvPath} (already exists)`);
    }
  }
...
export async function runOnboard(): Promise<void> {
  console.log("containerclaw onboard");
  console.log("===============\n");
  console.log("This wizard will set up your containerclaw configuration.\n");

  const home = Deno.env.get("HOME") ?? "";
  const defaultDataDir = join(home, ".containerclaw", "data");
  ...
  console.log("\nSetting up containerclaw...\n");
  await writeOnboardFiles(configPath, workspace, dataDir, configData, apiKey);

  const dotenvPath = join(dataDir, ".env");
  console.log("\nSetup complete!\n");
  console.log("Next steps:");
  console.log(
    `  1. ${apiKey ? "API key saved to " + dotenvPath : "Add your OpenRouter API key to " + dotenvPath}`,
  );
